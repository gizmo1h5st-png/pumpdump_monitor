...
def build_snapshot(symbol, klines, trades, settings: dict, pumpdump_info: dict = None) -> str:
    ...
    # Настройка цветов (используются автоматически для всех свечей на всех панелях)
    ...
    # УВЕЛИЧЕННЫЙ ОТСТУП СПРАВА (0.80 = 20% ширины под цену)
    ...
    ax_main.text(1.02, curr_price, f"{curr_price:.6f}", transform=ax_main.get_yaxis_transform(),
