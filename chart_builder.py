...
def klines_to_df(klines):
    try:
        ...
        # Формируем структуру данных Bybit V5
        processed_data = []
        for k in klines:
            if len(k) >= 6:
                processed_data.append([
                    ...
                    float(k[4]), # c
                    float(k[5]), # v
                    float(k[6]) if len(k) > 6 else float(k[5]) * float(k[4]) # turnover
                ])
        
        raw_df = pd.DataFrame(processed_data, columns=["Date","Open","High","Low","Close","Volume","Turnover"])
        raw_df["Date"] = pd.to_datetime(raw_df["Date"], unit="ms")
        raw_df.set_index("Date", inplace=True)
        df = raw_df.sort_index() # От старых к новым
...
def build_snapshot(symbol, klines, trades, settings: dict, pumpdump_info: dict = None) -> str:
    ...
    ax_main.text(1.03, curr_v, f"{curr_v:.6f}", transform=ax_main.get_yaxis_transform(),
