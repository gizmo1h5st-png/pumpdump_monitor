...
def klines_to_df(klines):
    try:
        ...
        # ПРАВИЛЬНЫЙ РАСЧЕТ CVD (Свечи дельты)
        diff = (df['High'] - df['Low']).replace(0, 1)
        df['RawDelta'] = (((df['Close'] - df['Low']) / diff) * df['Volume']) - (df['Volume'] / 2)
        df['CVD_Close'] = df['RawDelta'].cumsum()
        df['CVD_Open'] = df['CVD_Close'].shift(1).fillna(0)
...
def build_snapshot(symbol, klines, trades, settings: dict, pumpdump_info: dict = None) -> str:
    ...
    bg_color = '#0b0e11'; text_color = '#707a8a'
    ...
    if settings.get('show_delta', 1):
        c_df = df[['CVD_Open', 'CVD_High', 'CVD_Low', 'CVD_Close']].copy(); c_df.columns = ['Open', 'High', 'Low', 'Close']
    ...
    fig, axlist = mpf.plot(df[['Open', 'High', 'Low', 'Close']], type='candle', style=s, volume=False, addplot=ap, figsize=(15, 10 + cur_p*2),
    ...
    # --- УМНЫЙ HEATMAP (ТОЛЬКО НЕСНЯТАЯ ЛИКВИДНОСТЬ) ---
    if settings.get('show_heatmap', 1):
        ...
        for i in range(len(df)):
            ...
            # Показываем только уровни, которые сейчас ВНЕ текущей цены свечи
            mask = (price_bins[:-1] >= l) & (price_bins[1:] <= h)
            density[mask, i] = v
            
        # Гасим ликвидность, через которую цена уже прошла недавно
        profile = np.sum(density, axis=1)
        ax_main.imshow(density, interpolation='gaussian', origin='lower', aspect='auto',
                       extent=[0, len(df)-1, p_min, p_max], cmap='inferno', alpha=0.2, zorder=0)
        
        # Метки только для актуальных уровней (выше и ниже цены)
        top_idx_up = np.argmax(profile[price_bins[:-1] > curr_p]) + np.sum(price_bins[:-1] <= curr_p)
        top_idx_down = np.argmax(profile[price_bins[:-1] < curr_p])
        
        for idx in [top_idx_up, top_idx_down]:
            if idx < len(price_bins)-1:
                l_price = (price_bins[idx] + price_bins[idx+1]) / 2
                l_val = profile[idx] * l_price
                ax_main.text(len(df)-1, l_price, f" {human_format(l_val)}$", color='#f0b90b', 
                             fontsize=8, fontweight='bold', va='center', ha='left',
                             bbox=dict(facecolor='#0b0e11', alpha=0.6, edgecolor='none', pad=1))

    for i in range(0, len(axlist), 2):
        ax = axlist[i]
        ax.yaxis.tick_right(); ax.yaxis.set_label_position("right")
        if i == 0: ax.yaxis.set_major_formatter(mticker.FormatStrFormatter('%g'))
    ...
    if res_p > 0:
        ax_main.axhline(y=res_p, color='#f0b90b', linewidth=2, alpha=0.6, zorder=10)
    ...
    ax_main.text(0, 1.05, f"{symbol}   {pumpdump_info['change_percent']:+.2f}%" if pumpdump_info else symbol, 
                 transform=ax_main.transAxes, fontsize=20, fontweight='bold', color='white')
